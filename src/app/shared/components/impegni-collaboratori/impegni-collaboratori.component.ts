...
@Component({
  selector: 'app-impegni-collaboratori',
  templateUrl: './impegni-collaboratori.component.html',
  styleUrls: ['./impegni-collaboratori.component.scss'],
})
export class ImpegniCollaboratoriComponent implements OnInit {

  @Input() set impegnoDoc(value: ImpegnoCollaboratore) {
    this._impegnoDoc = value;
    //Reimposto gli attributi addizionali
    this.setAdditionalAttribute();
  }

  @Output() OnClickItem = new EventEmitter<ImpegnoCollaboratore>();


  _impegnoDoc: ImpegnoCollaboratore = new ImpegnoCollaboratore(true);
  iconName: string;
  iconColor: string; 
